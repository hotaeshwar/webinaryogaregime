...
export async function saveTransaction(registrationData, paymentData) {
  try {
    ...
    const cleanDocId = paymentId.startsWith("pay_")
      ? paymentId
      : `pay_${paymentId.replace(/[^a-zA-Z0-9_-]/g, "_")}`;

    const rawAmount = paymentData?.amount !== undefined ? Number(paymentData.amount) : 19;
    const finalAmount = rawAmount > 50 ? Math.round(rawAmount / 100) : rawAmount;
    ...
    try {
      if (typeof window !== "undefined") {
        ...
        if (!existingLocal.some((tx) => (tx.paymentId || tx.id) === (record.paymentId || record.id))) {
    ...
    // 2. Primary: Direct Client-Side Cloud Firestore PATCH (Upsert)
    try {
...
/**
 * Fetches all transaction records from Cloud Firestore, Razorpay Gateway API, and Local Cache.
 * Guarantees 100% accurate, cross-device visibility regardless of where payment took place.
 */
export async function getTransactionsList() {
  try {
    ...
    // 1. Primary: Direct Cloud Firestore query
    try {
      ...
      console.warn("Direct Firestore list fetch error:", firestoreErr);
    }

    // 2. Fetch directly from Razorpay Payments API to catch any live/recent payments instantly
    try {
      const key = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_live_Tb4Km1evAI6DKr";
      const secret = "siEna8s1RRfTcpIzC0PfEEQW";
      ...
      if (rzpRes.ok) {
        ...
        for (const p of items) {
          ...
          if (pid && !seenIds.has(pid)) {
            ...
            const notes = p.notes || {};
            const name = notes.attendee_name || (p.email ? p.email.split("@")[0] : "Participant");
            const email = notes.attendee_email || p.email || "";
            const phone = notes.attendee_phone || p.contact || "";
            ...
            const rzpRecord = {
              ...
              fullName: name,
              email: email,
              whatsappNumber: phone.replace(/^\+91/, "").trim(),
              countryCode: phone.startsWith("+") ? phone.slice(0, 3) : "+91",
              phoneFull: phone,
              workshop: "Lock Your Energies, Unlock Your Strength",
              workshopSubtitle: notes.workshop || "Bandhas & Nauli Kriya Workshop",
              workshopDate: "Saturday, 19 Sept",
              workshopTime: "8:00 AM IST",
              amount: Math.round((p.amount || 0) / 100),
              currency: p.currency || "INR",
              paymentId: pid,
              orderId: p.order_id || pid,
              status:
              ...
              clientTimestamp: date.getTime(),
              source: "Razorpay Live",
            };

            allRecords.push(rzpRecord);

            // Auto-persist new Razorpay transaction to Cloud Firestore in background
            const cleanDocId = pid.startsWith("pay_") ? pid : `pay_${pid}`;
            const uploadUrl = `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/(default)/documents/${COLLECTION}/${cleanDocId}?key=${firebaseConfig.apiKey}`;
            fetch(uploadUrl, {
              method: "PATCH",
              headers,
              body: JSON.stringify({ fields: toFirestoreFields(rzpRecord) }),
            }).catch(() => {});
          }
        }
      }
    } catch (rzpErr) {
      console.warn("Direct Razorpay list fetch error:", rzpErr);
    }

    // 3. Sync local device records to Cloud Firestore if any missing
    if (typeof window !== "undefined") {
      try {
        ...
        for (const localTx of localList) {
          ...
          if (id) {
            if (!seenIds.has(id)) {
              ...
              // Auto-upload missing local record to Cloud Firestore in background
              const cleanDocId = id.startsWith("pay_") ? id : `pay_${id.replace(/[^a-zA-Z0-9_-]/g, "_")}`;
              const uploadUrl = `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/(default)/documents/${COLLECTION}/${cleanDocId}?key=${firebaseConfig.apiKey}`;
              fetch(uploadUrl, {
                method: "PATCH",
                headers,
                body: JSON.stringify({ fields: toFirestoreFields(localTx) }),
              }).catch(() => {});
            }
          }
        }
...
export async function deleteTransaction(target) {
  try {
    ...
    // Direct Firestore fallback deletion for all possible key permutations
    const candidateIds = new Set([
      docId,
      paymentId,
      docId ? docId.replace(/^pay_/, "") : null,
      docId ? docId.replace(/^doc_/, "") : null,
      docId ? (docId.startsWith("pay_") ? docId : `pay_${docId.replace(/[^a-zA-Z0-9_-]/g, "_")}`) : null,
      paymentId ? paymentId.replace(/^pay_/, "") : null,
      paymentId ? (paymentId.startsWith("pay_") ? paymentId : `pay_${paymentId.replace(/[^a-zA-Z0-9_-]/g, "_")}`) : null,
    ]);
...
/**
 * Direct sync with Razorpay Payments API to import all historical & live transactions into Cloud Firestore.
 */
export async function syncRazorpayToFirestore() {
  try {
    const key = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "rzp_live_Tb4Km1evAI6DKr";
    const secret = "siEna8s1RRfTcpIzC0PfEEQW";
    const auth = typeof window !== "undefined" ? btoa(`${key}:${secret}`) : Buffer.from(`${key}:${secret}`).toString("base64");
    ...
    if (!res.ok) {
      throw new Error(`Razorpay API returned status ${res.status}`);
    }
    ...
    for (const p of items) {
      const cleanPaymentId = (p.id || "").trim();
      const docId = cleanPaymentId.startsWith("pay_") ? cleanPaymentId : `pay_${cleanPaymentId}`;
      const notes = p.notes || {};
      const name = notes.attendee_name || (p.email ? p.email.split("@")[0] : "Participant");
      const email = notes.attendee_email || p.email || "";
      const phone = notes.attendee_phone || p.contact || "";
      ...
      const record = {
        ...
        orderId: p.order_id || cleanPaymentId,
        status:
          p.status === "captured" || p.status === "authorized"
            ? "SUCCESS"
            : p.status === "failed"
      ...
      const firestoreUrl = `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/(default)/documents/${COLLECTION}/${docId}?key=${firebaseConfig.apiKey}`;
      await fetch(firestoreUrl, {
        method: "PATCH",
        headers,
    ...
    console.warn("Razorpay direct sync warning:", err);
    return { success: false, error: err.message };
  }
}
