...
export async function saveTransaction(registrationData, paymentData) {
  try {
    ...
    // 3. Backup: Save to Firebase Realtime Database
    const rtdbUrls = [
...
/**
 * Fetches all transaction records from Cloud Firestore, Realtime DB, and Local Cache.
...
export async function getTransactionsList() {
  try {
    ...
    // 2. Secondary: Fetch from Firebase Realtime Database backup
    const rtdbUrls = [
      `https://${firebaseConfig.projectId}-default-rtdb.firebaseio.com/registrations.json`,
      `https://${firebaseConfig.projectId}.firebaseio.com/registrations.json`,
    ];
    for (const rtdbUrl of rtdbUrls) {
      try {
        const rtdbRes = await fetch(rtdbUrl, { cache: "no-store" });
        if (rtdbRes.ok) {
          const rtdbData = await rtdbRes.json();
          if (rtdbData && typeof rtdbData === "object") {
            Object.entries(rtdbData).forEach(([docKey, tx]) => {
              if (tx && typeof tx === "object") {
                const id = tx.paymentId || tx.id || docKey;
                if (id && !seenIds.has(id)) {
                  seenIds.add(id);
        ...
        for (const localTx of localList) {
          ...
          if (id) {
            if (!seenIds.has(id)) {
              ...
              const cleanDocId = `pay_${id.replace(/[^a-zA-Z0-9_-]/g, "_")}`;
...
export async function syncRazorpayToFirestore() {
  try {
    ...
    for (const p of items) {
      const docId = p.id.startsWith("pay_") ? p.id : `pay_${p.id}`;
      const notes = p.notes || {};
